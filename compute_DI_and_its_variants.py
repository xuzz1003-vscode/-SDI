...
def compute_all_metrics(id, refs, paper_to_puby, time_span):
    try:
        ...
        for offset in range(time_span):
            ...
            i = focal_year_citations - ref_year_citations  # 仅引用目标论文的，在 [focal_year, focal_year+offset] 区间内，仅引用了 focal
...
def main():
    exp_ids_set=set()
    with open('/breakthrough_with_controls_cited_by_counts.json','r',encoding='utf-8') as f:
    ...
    result_path = '/control1_group_DI_and_its_variants.jsonl'
    exist_ids = load_exist_dois(result_path)
    id_to_refs, id_to_puby = get_papers_works(breakthrough_controls1_list)

    ids_to_process = [id for id in breakthrough_controls1_list if id not in exist_ids]
    print(f"总共需要处理的id数量：{len(breakthrough_controls1_list)},已经处理的id数量：{len(exist_ids)} 待处理 ID 数：{len(ids_to_process)}")
