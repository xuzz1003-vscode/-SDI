...
def plot_boxplot_comparison(metric, control_df1, control_df2, experiment_df, save_fig_path):
    ...
    add_significance(1, 2, y_max+0.05, pval_to_stars(p_value1))# sdi
    # add_significance(1, 2, y_max+0.005, pval_to_stars(p_value1),height) #di

    # 比较 exp vs control group 2
    y_max = max(np.max(exp_data), np.max(control_data2))
    add_significance(1, 3, y_max+0.13, pval_to_stars(p_value2))
    # add_significance(1, 3, y_max+0.01, pval_to_stars(p_value2),height)#di
...
def compare_experimental_and_controls(name,index_name,save_fig_path):
    # 使用示例
    if name=='DI' :
        experiment_df = jsonl_DI_index_to_dataframe('alt_disruption/control_group_one/results/experimental_papers_DI_and_its_variants.jsonl')
        control_df1=jsonl_DI_index_to_dataframe('alt_disruption/control_group_one/results/control_papers_DI_and_its_variants.jsonl')
        control_df2=jsonl_DI_index_to_dataframe('alt_disruption/control_group_two_new/results/control_papers_DI_and_its_variants_group2.jsonl')

    if name=='SDI':
        experiment_df = jsonl_SDI_index_to_dataframe('alt_disruption/control_group_one/results/experimental_papers_SDI_and_its_variants.jsonl')
        control_df1=jsonl_SDI_index_to_dataframe('alt_disruption/control_group_one/results/control_papers_SDI_and_its_variants.jsonl')
        control_df2=jsonl_SDI_index_to_dataframe('alt_disruption/control_group_two_new/results/control_papers_SDI_and_its_variants_group2.jsonl')
    ...
    plot_boxplot_comparison(index_name,control_df1, control_df2,experiment_df ,save_fig_path )
if __name__ == '__main__':

    compare_experimental_and_controls('SDI','SDI_xing','alt_disruption/control_group_two_new/figs/compare_focal_control_SDI_xing_box_new.png')
