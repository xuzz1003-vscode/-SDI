...
def compute_metrics(focal_paper, puby, refs, user_mentions):
    ...
    for i in range(time_gap + 1):
        ...
        k_set= set(all_ref_user_ids) - set(focal_user_ids) # 仅提及参考文献的用户数
        ni = len(i_set) 
        nj = len(j_set)  
        nk = len(k_set)  
        total = ni + nj + nk  # 总用户数
...
def main():
    """主函数，计算所有论文的指标并保存结果"""
    # 加载Twitter提及数据
    twitter_path = 'alt_disruption/final_run_experiment/data/all_papers_social_data.jsonl'
    ...
    # 设置结果保存路径
    save_path = 'alt_disruption/final_run_experiment/results/experimental_groups_SDI_metrics.jsonl'
