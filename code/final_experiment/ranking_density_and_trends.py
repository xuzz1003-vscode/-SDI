...
def plot_single_density(df, index_list, time):
    ...
    plt.tight_layout()
    plt.savefig(
        f'alt_disruption/final_run_experiment/figs/ranking_density_DI_{time}.png',
        dpi=300,
        bbox_inches='tight'
    )
    plt.close()
...
def plot_combined_means(all_means, index_list):
    ...
    plt.ylabel('Mean ranks of experimental groups')
      # 设置y轴为对数尺度
    plt.yscale('log')
    ...
    plt.tight_layout()
    plt.savefig('alt_disruption/final_run_experiment/figs/mean_rank_trend.png', dpi=300, bbox_inches='tight')
if __name__ == '__main__':
    # for y in range(0,3,2):
    #     results=rank_top(y)
    #     plot_single_density(results,['DI','DI5','mDI'], y)
    ...
    for y in range(0, 11):  # 假设年份为0, 2
        results = rank_top(y)  # 获取该年份的数据
        year_mean = df_mean(results)  # 计算均值
