...
def get_summary(df):
    ...
    # 计算基本统计量
    stats = {
        'Mean': round(numeric_df.mean(), 4),
        'Std': round(numeric_df.std(), 4),
        'Min': round(numeric_df.min(), 4),
        '25%': round(numeric_df.quantile(0.25), 4),
        'Median': round(numeric_df.median(), 4),
        '75%': round(numeric_df.quantile(0.75), 4),
        'Max': round(numeric_df.max(), 4),
        'Variance': round(numeric_df.var(), 4)
    }
...
def draw_corr(data):
    ...
    plt.yticks(ticks=[i+0.5 for i in np.arange(len(labels))], labels=labels, rotation=0, fontsize=11,va='center')


    plt.tight_layout()  # 使布局紧凑
    plt.savefig(f'alt_disruption/final_run_experiment/figs/all_data_corr.png',dpi=300, bbox_inches='tight')
...
def compare_experimental_and_controls():
    ...
    experiment_SDI = get_all_last_metrics(experiment_df_SDI)
    experiment_DI=get_all_last_metrics(experiment_df_DI)
    ...
    # get_summary(focal_merge_df)
    
    print('---------------control groups----------------------------------')
    control_DI= get_all_last_metrics(control_df_DI)
    control_SDI= get_all_last_metrics(control_df_SDI)
    ...
    # get_summary(control_merge_df)
    print('------------------计算相似性----------------------------------')
    all_data=pd.concat([focal_merge_df,control_merge_df])
