...
from compare_control_focal_papers_DI_8 import jsonl_DI_index_to_dataframe, jsonl_SDI_index_to_dataframe
...
def merge_data(time):
    ...
    # 确保关键列没有缺失值
    required_cols1 = ['DOI', 'label']   # 添加其他必要列
    ...
    # 确保关键列没有缺失值
    required_cols2 = ['DOI', 'label']   # 添加其他必要列
    all_data2 = all_data2.dropna(subset=required_cols2)
    
    print(f"最终样本量: {len(all_data1)}")
    print(f"标签分布:\n{all_data1['label'].value_counts()}")
    print(f"缺失值检查:\n{all_data1.isnull().sum()}")
    print(f"缺失值标签分布:\n{all_data1.isnull()['label'].value_counts()}")
...
if __name__ == '__main__':
    ...
    result_no_control.to_csv("alt_disruption/control_group_two_new/figs/control1_no_citation_nonobel.csv")
    ...
    result_with_control.to_csv("alt_disruption/control_group_two_new/figs/control1_with_citation_nonobel.csv")